...
const SeedEditPage: React.FC = () => {
  const { getUrl, setUrl, clear } = useSeedStore();
    ...
    const setupUrl = () => {
      try {
        ...
        if (savedUrl) {
          ...
          setVideoUrl(parsed.videoFile);
          return;
        }
        
        // store에 없으면 location.state 확인
        if (location.state) {
          const { videoFile } = location.state as LocationState;
          console.log('Using location sate: ', { videoFile });
  ...
  const generateThumbnail = async (video: HTMLVideoElement, time: number): Promise<string> => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        tempVideo.src = videoUrl;  // 단일 videoUrl 사용
  ...
  useEffect(() => {
    if (videosLoaded === 1 && duration > 0) {
      const generateThumbnails = async () => {
        ...
        try {
          const thumbnailPromises = await Promise.all(
            Array.from({ length: thumbnailCount }, (_, i) => 
              generateThumbnail(targetVideoRef.current!, i * interval)
            )
          );
          
          setThumbnails(thumbnailPromises.filter(thumb => thumb !== ''));
        } catch (error) {
